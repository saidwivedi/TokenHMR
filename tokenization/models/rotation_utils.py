...
def matrix_to_quaternion(matrix: torch.Tensor) -> torch.Tensor:
    ...
    quat_candidates = safe_zero_division(quat_by_rijk, (2.0 * q_abs[..., None].max(flr)))
...
def axis_angle_to_quaternion(axis_angle: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        quaternions with real part first, as tensor of shape (..., 4).
    """
    angles = torch.linalg.norm(axis_angle, ord=2, dim=-1, keepdim=True)
...
def quaternion_to_axis_angle(quaternions: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        Rotations given as a vector in axis angle form, as a tensor
            ...
            direction.
    """
    norms = torch.linalg.norm(quaternions[..., 1:], ord=2, dim=-1, keepdim=True)
    ...
    return safe_zero_division(quaternions[..., 1:], sin_half_angles_over_angles)
