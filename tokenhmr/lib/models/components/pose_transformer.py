...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, inp: torch.Tensor, *args, context=None, context_list=None):
        x = self.to_token_embedding(inp) # inp is zero matrix [4, 1, 1]
        b, n, _ = x.shape #[4, 1, 1024]
 
        x = self.dropout(x)
        x += self.pos_embedding[:, :n] #[4, 1, 1024] + [1, 1, 1024]
        # context [4, 192, 1280] is the actual features from backbone
