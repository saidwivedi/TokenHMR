...
INSTA_CORRUPT_TARS = {
    '002994.tar','002995.tar','002996.tar','002997.tar','002998.tar','002999.tar','003000.tar'
}
CORRUPT_TARS = {
    *{f'insta-train-vitpose-replicate/{k}' for k in INSTA_CORRUPT_TARS},
}
...
def expand_urls(urls):
    ...
    urls = [u for u in urls if not any(pattern in u for pattern in CORRUPT_TARS)]
    return urls
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # smpl_params = {'global_orient': self.body_pose[idx][:3].astype(np.float32),
        #                'body_pose': self.body_pose[idx][3:].astype(np.float32),
        #                'betas': self.betas[idx].astype(np.float32)
        #             }

        # has_smpl_params = {'global_orient': self.has_body_pose[idx],
        #                    'body_pose': self.has_body_pose[idx],
        #                    'betas': self.has_betas[idx]
        #                    }
    ...
    def load_tars_as_webdataset(cfg: CfgNode, urls, train: bool,
        ...
        urls = os.path.join(cfg.DATASETS.DATASET_DIR, urls)
        ...
        amass_poses_hist100_smooth = load_amass_hist_smooth(os.path.join(cfg.DATASETS.DATASET_DIR, 'amass_poses_hist100_SMPL+H_G.npy'))
        ...
        def ignore_smpl_params(item):
            item['data.pyd']['has_body_pose'] = np.array(float(False))
            item['data.pyd']['has_betas'] = np.array(float(False))
            return item
        ...
        if not use_smpl_params:
            dataset = dataset.map(lambda x: ignore_smpl_params(x))
